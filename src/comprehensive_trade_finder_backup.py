...
class ComprehensiveTradeUpFinder:
    ...
    async def initialize(self, sample_size: int = 1000) -> None:
        """Initialize with sample pricing data for quick start"""
        logger.info("Initializing Comprehensive Trade-up Finder...")
        
        # Get sample prices to start with
        ...
        self._cached_prices.update(sample_prices)
          # Build market data using comprehensive database + sample prices
    ...
    async def find_profitable_trades(self,
        ...
        logger.info(f"Searching for profitable trades (min_profit: ${min_profit})...")
        ...
        for input_rarity in rarities_to_check:
            ...
            for primary_collection in collections:
                ...
                # Try single collection trade-ups (10 items from same collection)
                if len(primary_input_skins) >= 1:  # Only need 1 type, we buy 10 copies
                    try:                        
                        ...
                        if result:
                            ...
                            if len(opportunities) >= limit:
                                logger.info(f"Found {len(opportunities)} opportunities, stopping early")
                                opportunities.sort(key=lambda x: x.expected_profit, reverse=True)
                                return opportunities[:limit]
                    except Exception as e:
                        logger.debug(f"Error in single collection trade-up: {e}")
                
                # Try mixed collection trade-ups (combining with other collections)
                for secondary_collection in collections:
                    if secondary_collection <= primary_collection:  # Avoid duplicates
                        continue
                    ...
                    # Try different split ratios (9+1, 8+2, 7+3, 6+4, 5+5)
        ...
        # Sort by profit and return top results        opportunities.sort(key=lambda x: x.expected_profit, reverse=True)
        return opportunities[:limit]
    ...
    async def _calculate_single_collection_tradeup(self,
        ...
        if missing_prices:
            ...
            # Validate and correct suspicious prices
        ...
        for condition in ['Factory New', 'Minimal Wear', 'Field-Tested', 'Well-Worn', 'Battle-Scarred']:
            ...
            output_float = input_float  # Average of 10 identical floats = same float
            
            # Filter outputs to only those possible with this float
            valid_outputs = self._filter_possible_outputs_by_float(marketable_outputs, output_float)
            if not valid_outputs:
                continue
              # Calculate expected output value using proper trade-up probabilities
            # For single collection: all outputs have equal probability (1/m_C)            # Only include outputs with valid pricing data
            valid_priced_outputs = []
            for output_skin in valid_outputs:
                output_price = self._cached_prices.get(output_skin['market_hash_name'])
                if output_price and float(output_price) > 0:  # Only include skins with valid prices
                    # Check if we have validated Steam pricing available
                    validation_status = self.db_manager.get_price_validation_status(output_skin['market_hash_name'])
                    if validation_status and validation_status.get('status') == 'valid':
                        ...
                        if steam_price and steam_price > 0:
                            valid_priced_outputs.append((output_skin, float(steam_price)))
                        else:
                            # Fallback to external price if Steam price is missing
                            valid_priced_outputs.append((output_skin, float(output_price)))
                    else:
                        # Use external price for unvalidated items
                        valid_priced_outputs.append((output_skin, float(output_price)))
            ...
            output_skin_objects = []
            for output_skin, output_price in valid_priced_outputs:
            ...
            if expected_profit > best_profit:
                ...
                best_result = TradeUpResult(
                    ...
                    min_output_price=min(Decimal(str(price)) for _, price in valid_priced_outputs)
                )
    ...
    async def _calculate_csfloat_validated_tradeup(self,
        ...
        # Scale floats to 0-1 range and calculate average
        input_skins_data = [cheapest_input] * 10  # All same skin
        average_scaled_float = self.csfloat_client.calculate_average_scaled_float(input_floats, input_skins_data)
        ...
        for output_name, output_data in output_listings_data.items():
            ...
            output_float = self.csfloat_client.calculate_output_float(average_scaled_float, output_skin)
            
            output_details.append({
                'name': output_name,
                'probability': probability,
                'csfloat_price': csfloat_price,
                'expected_float': output_float,
                'wear_condition': self.csfloat_client._get_wear_from_float(output_float),
                'skin_data': output_skin,
        ...
        return {
            ...
            'float_analysis': {
                ...
                'scaled_average_float': average_scaled_float,
                'input_wear_conditions': [self.csfloat_client._get_wear_from_float(f) for f in input_floats]
            },
    ...
    async def _validate_prices(self, prices: Dict[str, Decimal], skins: List[Dict]) -> Dict[str, Decimal]:
      ...
      def _is_marketable_skin(self, skin: Dict) -> bool:
        """Check if a skin is marketable (not souvenir, StatTrak, contraband, etc.)"""
      ...
      def _get_condition_float(self, skin: Dict) -> float:
        ...
        return condition_floats.get(condition, 0.265)
        return condition_floats.get(condition, 0.25)
    
    def _filter_possible_outputs_by_float(self, outputs: List[Dict], output_float: float) -> List[Dict]:
        """Filter outputs that are possible with the given float value"""
        valid_outputs = []
        for output in outputs:
            min_float = output.get('min_float', 0.0)
            max_float = output.get('max_float', 1.0)
            if min_float <= output_float <= max_float:
                valid_outputs.append(output)
        return valid_outputs
    
    def _has_price(self, market_hash_name: str) -> bool:
        """Check if we have pricing data for this item"""
    ...
    async def find_any_positive_return_trade_up(self,
                                               ...
                                               target_collections: Optional[List[str]] = None) -> Optional[Dict]:
        """
        Fallback method: Find any positive expected return trade-up using standard pricing
        """
        logger.info("Using fallback method to find positive expected return trade-ups...")
        ...
        for input_rarity in rarities_to_check:
            ...
            for collection in collections:
                ...
                # Try to find a positive return trade-up for this collection
                result = await self._calculate_fallback_tradeup(
                    collection, input_rarity, input_skins, selling_fee_rate, min_profit
                )
                
                if result:
                    logger.info(f"Found positive return trade-up in {collection} (fallback method)!")
                    return result
        
        logger.info("No positive expected return trade-ups found with fallback method")
        return None    async def _calculate_fallback_tradeup(self,
                                         collection: str,
                                         input_rarity: str,
                                         input_skins: List[Dict],
                                         selling_fee_rate: float,
                                         min_profit: float) -> Optional[Dict]:
        """Calculate trade-up using standard pricing data (fallback method)"""
        ...
        # Get possible output skins from the collection
        output_skins = [skin for skin in self.market_data.skins 
                       if skin.collection == collection and skin.rarity == output_rarity]
        
        if not output_skins:
            return None
        
        # Find cheapest marketable input skin
        cheapest_input = None
        cheapest_price = float('inf')
        
        for skin in input_skins:
            if self._is_marketable_skin(skin) and self._has_price(skin):
                price = self._get_price(skin)
        ...
        for output_skin in output_skins:
            if self._has_price(output_skin.__dict__):
                output_price = self._get_price(output_skin.__dict__)
                probability = 1.0 / len(output_skins)  # Equal probability for each output
                contribution = output_price * probability
                total_expected_value += contribution
                
                output_details.append({
                    'name': output_skin.name,
                    'collection': output_skin.collection,
                    'rarity': output_skin.rarity,
                    'probability': probability,
                    'market_price': output_price,
        ...
        # Calculate net value after selling fees
        net_expected_value = total_expected_value * (1 - selling_fee_rate)
        expected_profit = net_expected_value - input_cost
        
        # Check if trade-up has positive expected return
        if expected_profit <= min_profit:
            return None
        
        # Estimate float values
        input_float = self._get_condition_float(cheapest_input.get('condition_name', 'Field-Tested'))
        input_floats = [input_float] * 10
        
        return {
            'trade_up_type': 'fallback_analysis',
            'collection': collection,
            'input_rarity': input_rarity,
            'output_rarity': output_rarity,
            'validation_method': 'steam_market_pricing',
            'financial_summary': {
            ...
            'float_analysis': {
                'input_floats': input_floats,
                'average_input_float': input_float,
                'estimated_output_float': input_float,
                'input_wear_condition': cheapest_input.get('condition_name', 'Field-Tested')
            },
            'output_possibilities': output_details,
            'purchase_instructions': {
                ...
                'input_items': [{
                    ...
                    'steam_market_url': f"https://steamcommunity.com/market/listings/730/{cheapest_input['market_hash_name'].replace(' ', '%20')}"
                }],
                'total_cost': input_cost,
                'notes': 'Purchase all 10 items from Steam Community Market. Prices may fluctuate.'
            },
            'trade_up_instructions': {
                'items_needed': f"10x {cheapest_input['market_hash_name']}",
                'expected_outcome': f"1x random skin from {output_rarity} tier in {collection}",
                'success_probability': '100% (guaranteed trade-up)',
                'profit_probability': f"{(expected_profit / total_expected_value * 100):.1f}% chance of profit"
            }
        }
        ...
        for output_skin in marketable_outputs:
            ...
            if price and float(price) > 0:
                ...
                total_expected_value += float(price) * probability
                
                # Estimate output float based on input condition
                input_float = self._get_condition_float(cheapest_input)
                output_float = input_float  # Simplified: assume same float
                ...
                output_details.append({
                    ...
                    'price': float(price),
                    'expected_float': output_float,
                    'wear_condition': cheapest_input.get('condition_name', 'Field-Tested'),
                    'skin_data': output_skin
                })
        ...
        logger.debug(f"Fallback calculation: Cost=${input_cost:.2f}, Expected=${net_expected_value:.2f}, Profit=${expected_profit:.2f}")
        
        # Check if profitable
        if expected_profit <= min_profit:
            return None
        
        logger.info(f"✅ Positive expected return found! Expected profit: ${expected_profit:.2f}")
        
        # Estimate input floats (simplified)
        input_float = self._get_condition_float(cheapest_input)
        input_floats = [input_float] * 10
        
        # Return comprehensive trade-up analysis (fallback format)
        return {
            'trade_type': 'Single Collection Trade-up (Fallback)',
            'input_collection': collection,
            'input_rarity': input_rarity,
            'output_rarity': output_rarity,
            'input_skin': {
